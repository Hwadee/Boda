...
public class CustomerMessageService {
    ...
    public List<CustomerMessage> getcustomerInfo(String customerId) throws Exception {

        List<CustomerMessage> customerMessage = new LinkedList<>();

        if (Tool.isInteger(customerId)) { //优先以客户号搜索
            customerMessage.add(customerMapper.findCustomerById(Integer.parseInt(customerId)));
        }
        customerMessage.remove(null);
